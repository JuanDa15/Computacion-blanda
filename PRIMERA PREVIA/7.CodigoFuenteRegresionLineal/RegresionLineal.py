...
# Datos de trabajo
# -----------------------------------------------------------------
data = np.genfromtxt(os.path.join(DATA_DIR, "E:\JuanDavid\Computacion-blanda\Regresion Lineal\web_traffic.tsv"), delimiter="\t")
#Function genfromtxt is used to load data from a text file
# Se establece el tipo de dato
data = np.array(data, dtype=np.float64)

#print(data[:10]) #we print 10 lines of the document
#print(data.shape) #this statement print the dimention of the matriz 
...
x = data[:, 0]#We assing  the index column of the document
y = data[:, 1]#We assing the value data column of the document
...
#print("Número de entradas incorrectas:", np.sum(np.isnan(y))) #We print the number of nan's that we find in the value column
    ...
    if models:
        ...
        if mx is None:
            ...
            for model, style, color in zip(models, linestyles, colors):
                plt.plot(mx, model(mx), linestyle=style, linewidth=2, c=color) #Se usa la funcion plot para graficar los datos 
            #Se usa la funcion legend para mostrar un mensahe en la parte superior izquierda de la grafica
            plt.legend(["d=%i" % m.order for m in models], loc="upper left")
        #Se usa la funcion autoscale para que se ajuste automaticamente
        #La vista del eje a los datos
        plt.autoscale(tight=True)
        #Se usa la funcion ylim para obtener o establecer los limites 
        #'y' de los ejes. y de igual manera se usa la funcion xlim
        plt.ylim(ymin=0)
        if ymax:
            plt.ylim(ymax=ymax)
        if xmin:
            plt.xlim(xmin=xmin)
        #Con la funcion grid() lo que se hace es definir el estilo de la 
        #Cuadricula en este caso definimos como estilo de linea el -
        plt.grid(True, linestyle='-', color='0.75')
        #Por ultimo se usa la funcion savefig para guardar la figura.
        plt.savefig(fname)
...
# -----------------------------------------------------------------
#Se usa la funcion polifit para hacer un ajuste polinomial de los minimos cuadrados
#Para que esta retorne un vector de coeficientes p que minimiza el error al cuadrado
fp1, res1, rank1, sv1, rcond1 = np.polyfit(x, y, 1, full=True)
print("Parámetros del modelo fp1: %s" % fp1)#Se imprimen los parametros para el primer modelo
